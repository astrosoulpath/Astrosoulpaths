...
export class AuthService {
   ...
   * Even if LOCAL_OTP_ENABLED=true is accidentally set in production,
   * the local OTP/token flow remains disabled.
   */
  ...
  try {
    ...
    if (error) {
      this.logger.warn(
        `OTP send failed for ${phone}: ${error.message}`,
      );
    ...
    if (err instanceof BadRequestException) {
      this.logger.warn(
        `Send OTP handled failure for ${phone}: ${message}`,
      );
      throw err;
    }

    this.logger.error(
      `Send OTP failed for ${phone}: ${message}`,
    );
...
async verifyOtp(phone: string, token: string) {
  try {
    ...
    if (this.isLocalOtpEnabled()) {
      if (
        token !==
        (process.env.LOCAL_OTP_CODE || '123456')
      ) {
      ...
      const localSupabaseId =
        `local-supabase-${normalizedPhone.replace(/\D/g, '')}`;
  ...
  refreshToken: `local-refresh-token:${localSupabaseId}`,
    ...
    let data: Awaited<
      ReturnType<SupabaseService['verifyOtp']>
    >;
    ...
    try {
      ...
      const errorMessage =
        this.getErrorMessage(supabaseError).toLowerCase();
      ...
      throw new UnauthorizedException({
        success: false,
        message:
          this.getErrorMessage(supabaseError) ||
    ...
    if (
      ...
      this.logger.warn(
        `Verify OTP handled failure for ${phone}: ${message}`,
      );
      throw err;
    }
  ...
  async verifyAstrologerOtp(phone: string, token: string) {
    ...
    const user =
  await this.userService.enableAstrologerAccessBySupabaseId(
